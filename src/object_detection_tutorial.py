...
os.system('mkdir detection_result')
count=0
# In[13]:
...
for i in output_dict['detection_classes']:
    if i!=90:
        output_dict['detection_classes'].remove(i)
'''
...
for image_path in TEST_IMAGE_PATHS:
  ...
  for i in range(len(output_dict['detection_classes'])):
      if (output_dict['detection_classes'][i]==90):
          ...
          break
  for i in range(len(output_dict['detection_classes'])):
      if (output_dict['detection_classes'][i]==48):
          filtered_dict['detection_boxes'].append(output_dict['detection_boxes'][i])
          filtered_dict['detection_classes'].append(output_dict['detection_classes'][i])
          filtered_dict['detection_scores'].append(output_dict['detection_scores'][i])
          break 
  ...
  plt.imshow(image_np)
  #print("image is ",image_path,output_dict['detection_boxes'])  
  plt.savefig('detection_result/'+str(count)+'.png')
...
print ("信心分數\t",filtered_dict['detection_scores'])

print("物體位置\t",filtered_dict['detection_boxes'])
print("物體種類\t",filtered_dict['detection_classes'])
